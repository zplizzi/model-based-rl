...
def make_atari_config() -> MuZeroConfig:
    """Build a MuZeroConfig class with atari parameters."""
...
class Node(object):
    ...
    def __init__(self, prior: float):
        ...
        self.prior = prior
        # value_sum is the total backpropagated value of all simulations through this node
        self.value_sum = 0
        self.children = {}
...
class ActionHistory(object):
  ...
  Basically just a list of actions...
  """
...
class Game(object):
    ...
    def __init__(self, action_space_size: int, discount: float):
        ...
        self.history = []
        self.rewards = []
        # A list of MCTS policies, ordered from start to finish
        self.child_visits = []
    ...
    def store_search_statistics(self, root: Node):
        """Compute the MCTS improved policy and value at `root`."""
        # Compute softmax of visit counts for all children and store in
        # self.child_visits
        ...
        # The value of the node is the average of all the simulations through
        # the node
    ...
    def make_target(self, state_index: int, num_unroll_steps: int,
        ...
        """Make the value target for all nodes in the game history in the range
        [state_index, state_index + num_unroll_steps]."""
        ...
        for current_index in range(state_index,
            ...
            # The terminal value is either the discountd MCTS value estimate td_steps
            # past current_index, or 0 if it's past the end of the game
            ...
            # Append this value as a target, along with the reward at this
            # step and the policy at this step.
            if current_index < len(self.root_values):
                targets.append((value, self.rewards[current_index],
                                self.child_visits[current_index]))
            else:
                # States past the end of games are treated as absorbing states,
                # meaning they have 0 value, 0 reward, and ??? an empty policy?
                targets.append((0, 0, []))
        return targets
...
class ReplayBuffer(object):
    """Stores a fixed-size list of Game objects.
    ...
    def sample_batch(self, num_unroll_steps: int, td_steps: int):
        """Get a batch of game sequences of length `num_unroll_steps`.
        Returns a list of tuples:
        - The initial observation in the sequence
        - the sequence of actions
        - the result of g.`make_target`, which i think is the discounted value
        """
...
class Network(object):
    """Should implement the forward pass of all the models."""
...
def run_selfplay(config: MuZeroConfig, storage: SharedStorage,
    ...
    """The main loop for a game-play worker."""
    while True:
...
def ucb_score(config: MuZeroConfig, parent: Node, child: Node,
    ...
    """Compute the UCB for a state, action pair."""
    # pb_c_init is c1 in the paper
    # pb_c_base is c2 in the paper
    pb_c = math.log((parent.visit_count + config.pb_c_base + 1) /
                    config.pb_c_base) + config.pb_c_init
    pb_c *= math.sqrt(parent.visit_count) / (child.visit_count + 1)

    # The prior is the value prediction of the network at this state, action pair
    prior_score = pb_c * child.prior
    # child.value() is the average value of the rollouts through this action
...
def expand_node(node: Node, to_play: Player, actions: List[Action],
    ...
    # Get the un-normalized policy, and do a softmax to get action probabilities
...
def backpropagate(search_path: List[Node], value: float, to_play: Player,
    ...
    """Backpropagate up the tree.
        `search_path` should be a list of nodes from child to root
        `value` is the value prediction at the child
    TODO: should search_path include the child or not?
    """
    for node in search_path:
        ...
        # TODO: why is this not before the value_sum update?
...
def add_exploration_noise(config: MuZeroConfig, node: Node):
    ...
    noise = numpy.random.dirichlet([config.root_dirichlet_alpha] *
                                   len(actions))
