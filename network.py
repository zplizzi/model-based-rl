...
class DecoderNetwork(nn.Module):
    def __init__(self, args):
        ...
        # self.c2 = nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1)
        # self.c3 = nn.Conv2d(64, 32, kernel_size=3, stride=1, padding=1)
        # self.c4 = nn.Conv2d(32, args.color_channels, kernel_size=3, stride=1, padding=1)
...
class DynamicsNetwork(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.in_channels = 256 + num_action_layers

        # self.c1 = nn.Conv2d(self.in_channels, 256, kernel_size=3, padding=1)
        # # self.bn1 = nn.BatchNorm2d(256)
        # self.bn1 = nn.GroupNorm(num_groups=16, num_channels=256)

        # TODO: set back to 15 layers
        self.stack = nn.Sequential(*[
...
class DQNNetwork(nn.Module):
    def __init__(self, args):
        ...
        self.value_net = VectorHead(args, n_outputs=self.args.num_actions)
        self.i = 0
    ...
    def prediction(self, hidden):
        ...
        assert values.shape == (batch_size, self.args.num_actions)
        return values
...
class Network(nn.Module):
    def __init__(self, args):
        ...
        # self.value_net = VectorHead(args, n_outputs=1)
        # self.reward_net = VectorHead(args, n_outputs=1)
        # self.reward_net = VectorHead(args, n_outputs=self.args.num_actions)
        # self.policy_net = VectorHead(
        #     args, n_outputs=self.args.num_actions)
        self.i = 0
    ...
    def dynamics(self, hidden, action):
        # raw_reward = self.reward_net(hidden)
        # reward = select_from_axis(raw_reward, action, axis=1)
        # reward = self.reward_net(hidden).gather(1, action.unsqueeze(1))
        # reward = reward.reshape((self.args.batch_size, ))
        ...
        # TODO: make this more general, has to be divisible by 16
        # for group norm
        # num_action_layers = 16
        num_action_layers = math.ceil(self.args.num_actions / 16) * 16
        ...
        # # TODO: there's probably a better architecture here, that doesn't
        # # require info about the transition being stored in the final
        # # hidden state.
        # reward = self.reward_net(hidden).reshape((batch_size, ))
