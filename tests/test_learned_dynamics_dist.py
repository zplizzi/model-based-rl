...
from papers.muzero.algs.learned_dynamics_dist import Trainer, LearnedDynamicsPlayer, LearnedDynamicsNetwork
...
def test_dqn():
    ...
    network = LearnedDynamicsNetwork(args).to(args.device)
    model_storage = ModelStorage()
    # if args.prioritized_replay:
    #     replay_buffer = PrioritizedReplayBuffer(args)
    # else:
    replay_buffers = [ReplayBuffer(args)]
    # p = LearnedDynamicsPlayer(args, model_storage, replay_buffers, stats)
